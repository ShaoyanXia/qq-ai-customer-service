...
安装脚本支持 Ubuntu、Debian、CentOS、RHEL、Alibaba Cloud Linux 等常见服务器系统，会自动识别 `apt-get`、`dnf` 或 `yum`，并通过 `uv` 自动准备 AstrBot 所需的新版 Python。Python 依赖默认使用清华 PyPI 镜像，失败后会自动切到阿里云镜像和官方 PyPI。
