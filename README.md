...
安装脚本支持 Ubuntu、Debian、CentOS、RHEL、Alibaba Cloud Linux 等常见服务器系统，会自动识别 `apt-get`、`dnf` 或 `yum`，并通过 `uv` 自动准备 AstrBot 所需的新版 Python。Python 依赖默认优先安装预编译 wheel，避免 CentOS 7 老编译器编译失败；镜像会从清华切到阿里云和官方 PyPI。
